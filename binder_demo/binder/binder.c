...
struct binder_state *binder_open(unsigned mapsize)
{
    ...
    if (bs->fd < 0) {
        fprintf(stderr,"binder: cannot open device (%d)\n", errno);
        goto fail_open;
    }
    ...
    if (bs->mapped == MAP_FAILED) {
        fprintf(stderr,"binder: cannot map device (%d)\n", errno);
        goto fail_map;
    }

    printf("binder_open: bs->fd = %d\n", bs->fd);
    printf("binder_open: bs->mapped = %p\n", bs->mapped);
    printf("binder_open: bs->mapsize = %u\n", bs->mapsize);
        /* TODO: check version */
...
int binder_write(struct binder_state *bs, void *data, unsigned len)
{
    ...
    bwr.read_buffer = 0;
    printf("**********************************************\n");
    printf("binder_write ioctl data 0x%x\n", data);
    printf("binder_write ioctl read buffer  %ld, %ld, %ld\n", bwr.read_size, bwr.read_consumed, bwr.read_buffer);
    printf("binder_write ioctl write buffer %ld, %ld, 0x%x\n", bwr.write_size, bwr.write_consumed, bwr.write_buffer);
    printf("**********************************************\n");
    q = (char *)(bwr.write_buffer);
    p = (char *)data;
    printf("binder_write ioctl sizeof write_buffer %d\n", sizeof(bwr.write_buffer));
    printf("binder_write ioctl sizeof p %d\n", sizeof(p));
    printf("binder_write ioctl p %08p data %08p, q %08p\n", p, data, q);
    printf("**********************************************\n");
    printf("binder_write ioctl data[0] 0x%02x\n", *((char *)data));
    printf("binder_write ioctl p[0] 0x%02x\n", *p);
    printf("binder_write ioctl bwr.write_buffer[0] 0x%02x\n", *q);
    printf("**********************************************\n");
    printf("binder_write ioctl write_buffer[0] 0x%02x\n", *((char *)(data)));
    printf("binder_write ioctl write_buffer[1] 0x%02x\n", *((char *)(data)+1));
    printf("binder_write ioctl write_buffer[2] 0x%02x\n", *((char *)(data)+2));
    printf("binder_write ioctl write_buffer[3] 0x%02x\n", *((char *)(data)+3));
    printf("**********************************************\n");
    
    res = ioctl(bs->fd, BINDER_WRITE_READ, &bwr);
    printf("ioctl BINDER_WRITE_READ done\n");
    if (res < 0) {
        fprintf(stderr,"binder_write: ioctl failed (%d)\n", errno);
    }
    printf("binder_write return %d\n", res);
    return res;
}
                ...
                void *target, uint32_t code)
{
    ...
    for (;;) {
        ...
        if (res < 0) {
            fprintf(stderr,"binder: ioctl failed (%d)\n", errno);
            goto fail;
        }
