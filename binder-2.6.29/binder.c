...
#define IS_ALIGNEDX(x, a) (((x) & ((typeof(x))(a) - 1)) == 0)
...
//EXPORT_SYMBOL(unmap_kernel_range);

//extern void zap_page_range(struct vm_area_struct *vma, unsigned long address,
	...
	struct binder_transaction_data *tr, int reply)
{
    ...
    printk(KERN_INFO "binder_transaction: tr->target.handle 0x%x\n", (int)tr->target.handle);
    printk(KERN_INFO "binder_transaction: tr->target.code 0x%x\n", tr->code);
    printk(KERN_INFO "binder_transaction: tr.data_size %d\n", (int)tr->data_size);
    printk(KERN_INFO "binder_transaction: tr.tr.data.ptr.buffer %p\n", tr->data.ptr.buffer);
    printk(KERN_INFO "binder_transaction: tr.offsets_size %d\n", (int)tr->offsets_size);
    printk(KERN_INFO "binder_transaction: tr.tr.data.ptr.offsets %p\n", tr->data.ptr.offsets);
		if (tr->target.handle) {
			struct binder_ref *ref;
      ...
      printk(KERN_INFO "binder_transaction: binder_context_mgr_node 0x%p\n", binder_context_mgr_node);
    ...
    printk(KERN_INFO "binder_transaction: target_node->debug_id 0x%x\n", target_node->debug_id);
    ...
    printk(KERN_INFO "binder_transaction: tr->flags 0x%x\n", tr->flags);
  ...
  printk(KERN_INFO "binder_transaction: target_thread 0x%p\n", target_thread);
	if (target_thread) {
  ...
  printk(KERN_INFO "**********************************************\n");
  printk(KERN_INFO "binder_transaction: t->buffer->data 0x%p\n", t->buffer->data);
	if (t->buffer == NULL) {
		return_error = BR_FAILED_REPLY;
  ...
  printk(KERN_INFO "binder_transaction: t->buffer->data 0x%p tr->data_size %d ALIGNED %d\n", t->buffer->data, (int)tr->data_size, (int)ALIGN(tr->data_size, sizeof(void *)));
  ...
  printk(KERN_INFO "binder_transaction: offp %p tr->offsets_size 0x%x, t->buffer->data_size %d\n", offp, (int)tr->offsets_size, (int)t->buffer->data_size);
	off_end = (void *)offp + tr->offsets_size;
	for (; offp < off_end; offp++) {
		struct flat_binder_object *fp;
    printk(KERN_INFO "binder_transaction: size_t %ld *offp 0x%08x%08x > t->buffer->data_size - sizeof(*fp) 0x%08x%08x\n", sizeof(size_t), (int)(*offp>>32), (int)*offp, (int)((t->buffer->data_size - sizeof(*fp))>>32), (int)(t->buffer->data_size - sizeof(*fp)));
    printk(KERN_INFO "binder_transaction: t->buffer->data_size 0x%x < sizeof(*fp) 0x%lx\n", (int)t->buffer->data_size, sizeof(*fp));
    printk(KERN_INFO "binder_transaction: IS_ALIGNED %d\n", (int)IS_ALIGNED(*offp, sizeof(void *)));
    printk(KERN_INFO "**********************************************\n");
		if (*offp > (t->buffer->data_size - sizeof(*fp))) {
        printk(KERN_INFO "binder_transaction: *offp > t->buffer->data_size - sizeof(*fp)\n");
		}
		if (t->buffer->data_size < sizeof(*fp)) {
        printk(KERN_INFO "binder_transaction: t->buffer->data_size < sizeof(*fp)\n");
		}
		if (!IS_ALIGNED(*offp, sizeof(void *))) {
        printk(KERN_INFO "binder_transaction: !IS_ALIGNED(*offp, sizeof(void *))\n");
		}
    ...
    printk(KERN_INFO "binder_transaction: fp->type 0x%08x%08x\n", (int)(fp->type>>32), (int)fp->type);
    printk(KERN_INFO "binder_transaction: fp->binder %p\n", fp->binder);
    printk(KERN_INFO "binder_transaction: fp->cookie %p\n", fp->cookie);
		switch (fp->type) {
		case BINDER_TYPE_BINDER:
      printk(KERN_INFO "binder_transaction: BINDER_TYPE_BINDER\n");
		case BINDER_TYPE_WEAK_BINDER: {
			struct binder_ref *ref;
			struct binder_node *node = binder_get_node(proc, fp->binder);
      printk(KERN_INFO "binder_transaction: BINDER_TYPE_WEAK_BINDER %p\n", node);
			if (node == NULL) {
      ...
      printk(KERN_INFO "binder_transaction: binder_get_ref_for_node start\n");
      ...
      printk(KERN_INFO "binder_transaction: BINDER_TYPE_BINDER done\n");
		} break;
		case BINDER_TYPE_HANDLE:
      printk(KERN_INFO "binder_transaction: BINDER_TYPE_HANDLE\n");
		case BINDER_TYPE_WEAK_HANDLE: {
			struct binder_ref *ref = binder_get_ref(proc, fp->handle);
      printk(KERN_INFO "binder_transaction: BINDER_TYPE_WEAK_HANDLE %p\n", ref);
			if (ref == NULL) {
				binder_user_error("binder: %d:%d got "
  ...
  printk(KERN_INFO "binder_transaction: reply %d\n", reply);
	if (reply) {
  ...
  printk(KERN_INFO "binder_transaction: t->work.type = BINDER_WORK_TRANSACTION\n");
	t->work.type = BINDER_WORK_TRANSACTION;
	list_add_tail(&t->work.entry, target_list);
	tcomplete->type = BINDER_WORK_TRANSACTION_COMPLETE;
	list_add_tail(&tcomplete->entry, &thread->todo);
  printk(KERN_INFO "binder_transaction: wake_up_interruptible\n");
	if (target_wait)
		wake_up_interruptible(target_wait);
  printk(KERN_INFO "binder_transaction: return\n");
	return;
...
err_no_context_mgr_node:
  ...
  printk(KERN_INFO "binder_transaction: error return\n");
}
		    ...
		    void __user *buffer, int size, signed long *consumed)
{
  ...
  printk(KERN_INFO "binder_thread_write: start %p\n", ptr);
	while (ptr < end && thread->return_error == BR_OK) {
		if (get_user(cmd, (uint32_t __user *)ptr)) {
      printk(KERN_INFO "binder_thread_write: return -%d\n", EFAULT);
			return -EFAULT;
		}
    ...
    printk(KERN_INFO "binder_thread_write 1: cmd %d\n", cmd);
		switch (cmd) {
		case BC_INCREFS:
		case BC_ACQUIRE:
		case BC_RELEASE:
		case BC_DECREFS: {
			...
			switch (cmd) {
			case BC_INCREFS:
        printk(KERN_INFO "binder_thread_write 1: BC_INCREFS\n");
				debug_string = "IncRefs";
				binder_inc_ref(ref, 0, NULL);
				break;
			case BC_ACQUIRE:
        printk(KERN_INFO "binder_thread_write 1: BC_ACQUIRE\n");
				debug_string = "Acquire";
				binder_inc_ref(ref, 1, NULL);
				break;
			case BC_RELEASE:
        printk(KERN_INFO "binder_thread_write 1: BC_RELEASE\n");
				debug_string = "Release";
				binder_dec_ref(ref, 1);
				break;
			case BC_DECREFS:
			default:
        printk(KERN_INFO "binder_thread_write 1: BC_DECREFS\n");
        ...
        printk(KERN_INFO "binder_thread_write 1: BC_INCREFS_DONE\n");
		case BC_ACQUIRE_DONE: {
			void __user *node_ptr;
			void *cookie;
			struct binder_node *node;

      printk(KERN_INFO "binder_thread_write 1: BC_ACQUIRE_DONE\n");
        ...
        printk(KERN_INFO "binder_thread_write 1: BC_FREE_BUFFER\n");
		...
		case BC_TRANSACTION:
      printk(KERN_INFO "binder_thread_write 1: BC_TRANSACTION\n");
		case BC_REPLY: {
			struct binder_transaction_data tr;

      printk(KERN_INFO "binder_thread_write 1: BC_REPLY\n");
      ...
      printk(KERN_INFO "binder_thread_write 1: BC_REGISTER_LOOPER\n");
      ...
      printk(KERN_INFO "binder_thread_write 1: BC_ENTER_LOOPER\n");
			if (binder_debug_mask & BINDER_DEBUG_THREADS)
			...
			break;
		case BC_EXIT_LOOPER:
      printk(KERN_INFO "binder_thread_write 1: BC_EXIT_LOOPER\n");
			if (binder_debug_mask & BINDER_DEBUG_THREADS)
      ...
      printk(KERN_INFO "binder_thread_write 1: BC_REQUEST_DEATH_NOTIFICATION\n");
      ...
      printk(KERN_INFO "binder_thread_write 1: BC_CLEAR_DEATH_NOTIFICATION\n");
			if (get_user(target, (uint32_t __user *)ptr))
				return -EFAULT;
			ptr += sizeof(uint32_t);
      ...
      printk(KERN_INFO "binder_thread_write 1: BC_DEAD_BINDER_DONE\n");
			if (get_user(cookie, (void __user * __user *)ptr))
				return -EFAULT;
  ...
  printk(KERN_INFO "binder_thread_write: end\n");
	return 0;
}
	...
	void  __user *buffer, int size, signed long *consumed, int non_block)
{
	...
	printk(KERN_INFO "binder_thread_read: size %d, consumed %ld, buffer %p)\n", size, *consumed, buffer);
	if (*consumed == 0) {
...
retry:
	...
	printk(KERN_INFO "binder_thread_read: 2\n");
	thread->looper |= BINDER_LOOPER_STATE_WAITING;
	if (wait_for_proc_work)
		proc->ready_threads++;
	mutex_unlock(&binder_lock);
	printk(KERN_INFO "binder_thread_read: wait_for_proc_work %d\n", wait_for_proc_work);
	if (wait_for_proc_work) {
	  printk(KERN_INFO "binder_thread_read: thread->looper 0x%x\n", thread->looper);
		if (!(thread->looper & (BINDER_LOOPER_STATE_REGISTERED |
	    ...
	    printk(KERN_INFO "binder_thread_read: wait_event_interruptible 0 start\n");
			wait_event_interruptible(binder_user_error_wait, binder_stop_on_user_error < 2);
	    printk(KERN_INFO "binder_thread_read: wait_event_interruptible 0 done\n");
		}
	    ...
	    printk(KERN_INFO "binder_thread_read: wait_event_interruptible 1 start\n");
			ret = wait_event_interruptible_exclusive(proc->wait, binder_has_proc_work(proc, thread));
	    printk(KERN_INFO "binder_thread_read: wait_event_interruptible 1 done\n");
	} else {
		if (non_block) {
			if (!binder_has_thread_work(thread))
				ret = -EAGAIN;
		} else
	    printk(KERN_INFO "binder_thread_read: wait_event_interruptible 2 start\n");
			ret = wait_event_interruptible(thread->wait, binder_has_thread_work(thread));
	    printk(KERN_INFO "binder_thread_read: wait_event_interruptible 2 done\n");
	}
	...
	printk(KERN_INFO "binder_thread_read: 4)\n");
	while (1) {
		uint32_t cmd;
		struct binder_transaction_data tr;
		struct binder_work *w;
	...
	printk(KERN_INFO "binder_thread_read: 5, type %d)\n", w->type);
		switch (w->type) {
...
static long binder_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	printk(KERN_INFO "binder_ioctl 0: %d:%d %x %lx\n", proc->pid, current->pid, cmd, arg);
	...
	case BINDER_WRITE_READ: {
		...
		printk(KERN_INFO "binder: goto BINDER_WRITE_READ\n");
		if (size != sizeof(struct binder_write_read)) {
			printk(KERN_INFO "binder: invlalid size\n");
			ret = -EINVAL;
			goto err;
		}
		if (copy_from_user(&bwr, ubuf, sizeof(bwr))) {
			printk(KERN_INFO "binder: copy_from_user failure\n");
			ret = -EFAULT;
			goto err;
		}
		printk(KERN_INFO "binder: bwr.write (size %ld, consumed %ld, write_buff %ld)\n", bwr.write_size, bwr.write_consumed, bwr.write_buffer);
		...
		if (bwr.write_size > 0) {
			...
			if (ret < 0) {
			  printk(KERN_INFO "binder: binder_thread_write failure 1\n");
				bwr.read_consumed = 0;
				if (copy_to_user(ubuf, &bwr, sizeof(bwr))) {
			    printk(KERN_INFO "binder: copy_to_user failure 1\n");
					ret = -EFAULT;
				}
				goto err;
			}
		}
		printk(KERN_INFO "binder: bwr.read (size %ld, consumed %ld, read_buff %ld)\n", bwr.read_size, bwr.read_consumed, bwr.read_buffer);
		if (bwr.read_size > 0) {
			ret = binder_thread_read(proc, thread, (void __user *)bwr.read_buffer, bwr.read_size, &bwr.read_consumed, filp->f_flags & O_NONBLOCK);
			if (!list_empty(&proc->todo))
				wake_up_interruptible(&proc->wait);
			if (ret < 0) {
			  printk(KERN_INFO "binder: binder_thread_read failure 1\n");
				if (copy_to_user(ubuf, &bwr, sizeof(bwr))) {
			    printk(KERN_INFO "binder: copy_to_user failure 2\n");
					ret = -EFAULT;
				}
				goto err;
			}
		...
		if (copy_to_user(ubuf, &bwr, sizeof(bwr))) {
			printk(KERN_INFO "binder: copy_to_user failure 3\n");
			ret = -EFAULT;
			goto err;
		}
		break;
	...
	case BINDER_SET_MAX_THREADS:
		printk(KERN_INFO "binder: goto BINDER_SET_MAX_THREADS\n");
	...
	case BINDER_SET_CONTEXT_MGR:
		printk(KERN_INFO "binder: goto BINDER_SET_CONTEXT_MGR\n");
	...
	case BINDER_THREAD_EXIT:
		printk(KERN_INFO "binder: goto BINDER_THREAD_EXIT\n");
		if (binder_debug_mask & BINDER_DEBUG_THREADS)
		...
		break;
	case BINDER_VERSION:
		printk(KERN_INFO "binder: goto BINDER_VERSION\n");
		...
		break;
	default:
		printk(KERN_INFO "binder: goto default\n");
		ret = -EINVAL;
		goto err;
	}
	ret = 0;
...
static int __init binder_init(void)
{
  ...
  printk(KERN_ERR "binder: binder_init start\n");
  ...
  printk(KERN_ERR "binder: binder_init end return %d\n", ret);
	return ret;
}
